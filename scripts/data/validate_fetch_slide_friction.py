#!/usr/bin/env python
"""Validate paired FetchSlide data before any model training."""
    ...
    with h5py.File(args.input, "r") as dataset:
        ...
        for pair_id in unique_pairs:
            ...
            if len(indices) != 2 or set(modes[indices].tolist()) != {0, 1}:
                raise RuntimeError(
                    f"Pair {pair_id} does not contain one low and one high mode"
                )
            ...
            if lengths[first] != lengths[second]:
                raise RuntimeError(f"Pair {pair_id} has unequal episode lengths")
            first_slice = slice(starts[first], ends[first])
            second_slice = slice(starts[second], ends[second])
            if not np.array_equal(
                dataset["action"][first_slice], dataset["action"][second_slice]
            ):
                raise RuntimeError(
                    f"Pair {pair_id} does not share one commanded strike sequence"
                )
            paired_start_error.append(
                np.max(
                    np.abs(
                        np.asarray(dataset["prev_qpos"][starts[first]])
                        - np.asarray(dataset["prev_qpos"][starts[second]])
                    )
                )
            )
            if not np.array_equal(
                dataset["goal_position"][starts[first]],
                dataset["goal_position"][starts[second]],
            ):
